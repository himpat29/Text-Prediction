...
Predict2 <- function(x) {
  ...
  if (length(xs)>= 3) {
    xs <- tail(xs,3)
    if (identical(character(0),head(quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4],2))){
      Predict2(paste(xs[2],xs[3],sep=" "))
    }
    else {mesg <<- "Next word is predicted using 4-gram."; quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4][2]}
  }
  else if (length(xs) == 2){
    xs <- tail(xs,2)
    if (identical(character(0),head(trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3],2))) {
      Predict2(xs[2])
    }
    else {mesg<<- "Next word is predicted using 3-gram."; trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3][2]}
  }
  else if (length(xs) == 1){
    xs <- tail(xs,1)
    if (identical(character(0),head(bigram[bigram$unigram == xs[1], 2],2))) {mesg<<-"No match found. Most common word 'the' is returned."; head("the",1)}
    else {mesg <<- "Next word is predicted using 2-gram."; bigram[bigram$unigram == xs[1],2][2]}
  }
...
shinyServer(function(input, output) {
    ...
    output$prediction2 <- renderPrint({
      result2 <- Predict2(input$inputString)
      result2
    });
